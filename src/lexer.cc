...
namespace Cpp {
    ...
    Lexer::Lexer(std::string_view input, size_t start_line)
            : m_input(input)
    ...
    constexpr std::string_view s_known_keywords[] = {
            "alignas",
            "alignof",
            "and",
            "and_eq",
    ...
    constexpr std::string_view s_known_types[] = {
            "Array",
            "Array",
            "Badge",
            "Bitmap",
    ...
    static bool is_keyword(std::string_view string)
    {
        static std::set<std::string> keywords;
        if (keywords.empty()) {
            keywords.insert(std::cbegin(s_known_keywords), std::cend(s_known_keywords));
        }
        return keywords.contains(std::string(string));
    }

    static bool is_known_type(std::string_view string)
    {
        static std::set<std::string> types;
        if (types.empty()) {
            types.insert(std::cbegin(s_known_types), std::cend(s_known_types));
        }
        return types.contains(std::string(string));
    }

    void Lexer::lex_impl(std::function<void(Token)> callback)
    {
        size_t token_start_index = 0;
        Position token_start_position;

        auto emit_single_char_token = [&](auto type) {
            callback(Token(type, m_position, m_position, m_input.substr(m_index, 1)));
            consume();
        };
        ...
        auto commit_token = [&](auto type) {
            if (m_options.ignore_whitespace && type == Token::Type::Whitespace)
                return;
            callback(Token(type, token_start_position, m_previous_position, m_input.substr(token_start_index, m_index - token_start_index)));
        };
        ...
        while (m_index < m_input.length()) {
            ...
            if (ch == '#') {
                ...
                auto directive = std::string_view{m_input.data() + directive_start, m_index - directive_start};
            ...
            if (size_t prefix = match_string_prefix('R'); prefix > 0 && peek(prefix) == '"') {
                ...
                while (peek() && peek() != '(')
                    consume();
                std::string_view prefix_string = m_input.substr(prefix_start, m_index - prefix_start);
                while (peek()) {
                    if (consume() == '"') {
                        assert(m_index >= prefix_string.length() + 2);
                        assert(m_input[m_index - 1] == '"');
                        if (m_input[m_index - 1 - prefix_string.length() - 1] == ')') {
                            std::string_view suffix_string = m_input.substr(m_index - 1 - prefix_string.length(), prefix_string.length());
            ...
            if (is_valid_first_character_of_identifier(ch)) {
                ...
                while (peek() && is_valid_nonfirst_character_of_identifier(peek()))
                    consume();
                auto token_view = std::string_view(m_input.data() + token_start_index, m_index - token_start_index);
